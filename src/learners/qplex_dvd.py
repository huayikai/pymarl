...
from modules.mixers.dvd_qplex import DMAQer_DVD 
...
class DVD_QPLEXLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer is not None:
            ...
            elif args.mixer == "dvd_qplex":
                self.mixer = DMAQer_DVD(args)
            else:
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,
        ...
        # --- [DVD 修改] 计算主网络 Q 值并收集 Hidden States ---
        mac_out = []
        hidden_states_list = [] # [DVD 新增]
        ...
        for t in range(batch.max_seq_length):
            ...
            # [DVD 新增] 收集 hidden_states
            # 注意: mac.hidden_states 通常是 (batch, n_agents, dim) 或 (batch * n_agents, dim)
            # 务必 .clone()
            if self.args.mixer == "dvd_qplex":
                cur_h = mac.hidden_states.view(batch.batch_size, self.args.n_agents, -1).clone()
                hidden_states_list.append(cur_h)

        mac_out = th.stack(mac_out, dim=1)  # Concat over time
        
        # [DVD 新增] 整理 hidden states
        hidden_states_main = None
        if self.args.mixer == "dvd_qplex":
            whole_hidden_states = th.stack(hidden_states_list, dim=1)
            # 主网络训练只用前 T-1 步 (与 actions 对应)
        ...
        # --- [DVD 修改] 计算目标网络 Q 值并收集 Hidden States ---
        target_mac_out = []
        target_hidden_states_list = [] # [DVD 新增]
        ...
        # [DVD 新增]
        whole_target_hidden_states = None
        if self.args.mixer == "dvd_qplex":
        ...
        if mixer is not None:
            # 计算 V(s) 部分 (is_v=True)，不需要 hidden_states
            ans_chosen = mixer(chosen_action_qvals, batch["state"][:, :-1], is_v=True)
            
            # 计算 Advantage 部分 (is_v=False)，需要 hidden_states (用于生成权重)
            # 注意: 这里传入 hidden_states_main
            if self.args.mixer == "dvd_qplex":
                ans_adv = mixer(chosen_action_qvals, batch["state"][:, :-1], actions=actions_onehot,
                                max_q_i=max_action_qvals, is_v=False, hidden_states=hidden_states_main)
            else:
                ans_adv = mixer(chosen_action_qvals, batch["state"][:, :-1], actions=actions_onehot,
            ...
            if self.args.double_q:
                ...
                # Target Advantage
                # 注意: 这里传入 whole_target_hidden_states (因为 target_mixer 处理的是全序列)
                if self.args.mixer == "dvd_qplex":
                    target_adv = self.target_mixer(target_chosen_qvals, batch["state"],
                                                   ...
                                                   hidden_states=whole_target_hidden_states)
                else:
                    target_adv = self.target_mixer(target_chosen_qvals, batch["state"],
                                                   actions=cur_max_actions_onehot,
                                                   max_q_i=target_max_qvals, is_v=False)
